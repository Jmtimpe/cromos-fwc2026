...
import { observeInventarioAmigo, sumarUnoAlInventario } from '../lib/inventario';
...
function VistaAmigo({ amigo, miInventario, miUsuario, onVolver }) {
  ...
  const [vistaActiva, setVistaActiva] = useState('matches');
  ...
  const analisis = useMemo(() => {
    if (!catalogo.length) {
      return { matchesParaMi: [], matchesParaEl: [], completo: catalogo };
    }
  ...
  const handleMarcarRecibido = async (cromo) => {
    ...
    setPedidoEnCurso(cromo.numero);
    const resultSumar = await sumarUnoAlInventario(miUsuario.uid, cromo.numero);
    if (resultSumar.success) {
      ...
      setMensajePedido({ tipo: 'error', texto: resultSumar.error });
    }
    setPedidoEnCurso(null);
    setTimeout(() => setMensajePedido({ tipo: '', texto: '' }), 4000);
  };
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center py-20">
        ...
        <p className="text-gray-400 text-sm uppercase tracking-widest">Cargando álbum...</p>
      </div>
    );
  }
    ...
    <div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
          <div>
            ...
            <p className="font-display font-bold text-2xl sm:text-3xl text-fwc-gold">
              {statsAmigo.porcentaje}<span className="text-sm sm:text-lg">%</span>
            </p>
          </div>
          <div>
            <p className="text-gray-400 text-[10px] sm:text-xs uppercase tracking-widest">Tiene</p>
            <p className="font-display font-bold text-2xl sm:text-3xl text-white">
              {statsAmigo.obtenidos}<span className="text-sm sm:text-base text-gray-500">/{statsAmigo.total}</span>
            </p>
          </div>
          <div>
            <p className="text-gray-400 text-[10px] sm:text-xs uppercase tracking-widest">Sus repes</p>
            <p className="font-display font-bold text-2xl sm:text-3xl text-fwc-neon">{statsAmigo.repes}</p>
          </div>
          <div>
            <p className="text-gray-400 text-[10px] sm:text-xs uppercase tracking-widest">¡Para ti!</p>
            <p className="font-display font-bold text-2xl sm:text-3xl text-fwc-accent">{analisis.matchesParaMi.length}</p>
          </div>
        </div>
      </div>
        ...
        <div className="fwc-card p-5 mb-6 bg-gradient-to-r from-fwc-gold/10 to-fwc-neon/10 border-fwc-gold">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="bg-fwc-bg/50 rounded-lg p-3">
              <p className="text-fwc-gold text-xs uppercase tracking-wider mb-1">Te puede dar</p>
              <p className="font-display font-black text-3xl text-fwc-gold">{analisis.matchesParaMi.length}</p>
              <p className="text-gray-400 text-xs">cromos que necesitas</p>
            </div>
            <div className="bg-fwc-bg/50 rounded-lg p-3">
              <p className="text-fwc-neon text-xs uppercase tracking-wider mb-1">Le puedes dar</p>
              <p className="font-display font-black text-3xl text-fwc-neon">{analisis.matchesParaEl.length}</p>
      ...
      {mensajePedido.texto && (
        <div className={`mb-4 rounded-lg p-3 text-sm flex items-center gap-2 ${
          ...
          {mensajePedido.tipo === 'exito' ? <CheckCircle2 className="w-4 h-4" /> : <AlertCircle className="w-4 h-4" />}
          {mensajePedido.texto}
        </div>
      )}
      ...
      <div className="flex flex-wrap gap-2 mb-4">
        <button
          ...
          className={`px-4 py-2 rounded-lg border font-display font-bold uppercase tracking-wider text-xs transition-all flex items-center gap-2 ${
            vistaActiva === 'completo' ? 'bg-white text-fwc-bg border-white' : 'bg-fwc-card border-fwc-border text-gray-400 hover:border-fwc-gold hover:text-white'
          }`}
        >
        ...
        <button
          ...
          className={`px-4 py-2 rounded-lg border font-display font-bold uppercase tracking-wider text-xs transition-all flex items-center gap-2 ${
            vistaActiva === 'matches' ? 'bg-fwc-gold text-fwc-bg border-fwc-gold' : 'bg-fwc-card border-fwc-border text-gray-400 hover:border-fwc-gold hover:text-white'
          }`}
        >
        ...
        <button
          ...
          className={`px-4 py-2 rounded-lg border font-display font-bold uppercase tracking-wider text-xs transition-all flex items-center gap-2 ${
            vistaActiva === 'paraEl' ? 'bg-fwc-neon text-fwc-bg border-fwc-neon' : 'bg-fwc-card border-fwc-border text-gray-400 hover:border-fwc-neon hover:text-white'
          }`}
        >
        ...
        <div className="flex justify-end mb-4">
          <button
            ...
            className={`px-3 py-2 rounded-lg border text-xs font-bold uppercase tracking-wider transition-all flex items-center gap-1 ${
              soloMatches ? 'bg-fwc-gold text-fwc-bg border-fwc-gold' : 'bg-fwc-card text-gray-400 border-fwc-border hover:border-fwc-gold hover:text-fwc-gold'
            }`}
          >
          ...
          {Object.keys(catalogoAgrupado).map((seccion) => {
            ...
            Object.values(catalogoAgrupado[seccion]).forEach((cromos) => {
              cromos.forEach((c) => {
                ...
                if ((inventarioAmigo[c.numero] || 0) >= 2 && (miInventario[c.numero] || 0) === 0) matchesEnSeccion++;
              });
            });
            ...
            return (
              <div key={seccion} className="fwc-card overflow-hidden">
                <button
                  onClick={() => setSeccionAbierta(seccionEstaAbierta ? null : seccion)}
                  className="w-full p-4 flex items-center justify-between hover:bg-fwc-bg/30 transition-colors"
                >
                  <div className="flex items-center gap-3">
                    <span className={`text-fwc-gold transition-transform ${seccionEstaAbierta ? 'rotate-90' : ''}`}>▶</span>
                    <h3 className="font-display font-bold text-lg text-white uppercase tracking-wider">{seccion}</h3>
                  </div>
                  <div className="flex items-center gap-3">
                    {matchesEnSeccion > 0 && (
                    ...
                    <span className="text-gray-400 text-sm font-mono">{tieneSeccion}/{totalSeccion}</span>
                  </div>
                </button>
                {seccionEstaAbierta && (
                  <div className="border-t border-fwc-border">
                    {Object.keys(catalogoAgrupado[seccion]).map((equipo) => {
                      ...
                      cromosEquipo.forEach((c) => {
                        ...
                        if ((inventarioAmigo[c.numero] || 0) >= 2 && (miInventario[c.numero] || 0) === 0) matchesEquipo++;
                      });
                      if (soloMatches && matchesEquipo === 0) return null;
                      const equipoEstaAbierto = equipoAbierto === `${seccion}-${equipo}`;
                      return (
                        <div key={equipo} className="border-b border-fwc-border last:border-b-0">
                          <button
                            onClick={() => setEquipoAbierto(equipoEstaAbierto ? null : `${seccion}-${equipo}`)}
                            className="w-full p-3 pl-8 flex items-center justify-between hover:bg-fwc-bg/30 transition-colors"
                          >
                            <div className="flex items-center gap-3">
                              <span className={`text-fwc-neon text-xs transition-transform ${equipoEstaAbierto ? 'rotate-90' : ''}`}>▶</span>
                              <Bandera iso={eqInfo.iso} size="sm" />
                              <span className="font-display font-bold text-white text-sm">{eqInfo.nombre}</span>
                            </div>
                            <div className="flex items-center gap-2">
                              ...
                              <span className="text-gray-500 text-xs font-mono">{tieneEquipo}/{cromosEquipo.length}</span>
                            </div>
                          </button>
                          {equipoEstaAbierto && (
...
function ListaMatches({ cromos, inventarioRef, tipo, mensaje, onPedir, onMarcarRecibido, onCancelar, obtenerPedidoActivo, pidiendoEsteCromo }) {
  if (cromos.length === 0) {
    return (
        ...
        return (
          <div key={equipo} className="fwc-card p-4">
            <div className="flex items-center gap-3 mb-3 pb-3 border-b border-fwc-border">
              ...
              <span className="font-display font-bold text-white text-base">{eqInfo.nombre}</span>
              <span className={`ml-auto text-xs font-bold px-2 py-1 rounded ${
                tipo === 'paraMi' ? 'bg-fwc-gold/20 border border-fwc-gold/40 text-fwc-gold' : 'bg-fwc-neon/20 border border-fwc-neon/40 text-fwc-neon'
              }`}>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
              {lista.map((cromo) => (
                <CromoMatch
                  ...
                  pedidoActivo={obtenerPedidoActivo ? obtenerPedidoActivo(cromo.numero) : null}
...
function CromoMatch({ cromo, cantidad, tipo, onPedir, onMarcarRecibido, onCancelar, pedidoActivo, pidiendoEsteCromo }) {
  ...
  let cardStyle;
  if (tienePedidoAprobado) cardStyle = 'bg-gradient-to-br from-green-500/15 to-fwc-gold/10 border-green-500 shadow-lg shadow-green-500/20';
  else if (tienePedidoPendiente) cardStyle = 'bg-fwc-card border-fwc-gold/40';
  else if (tipo === 'paraMi') cardStyle = 'bg-gradient-to-br from-fwc-gold/20 to-fwc-accent/10 border-fwc-gold shadow-lg shadow-fwc-gold/20 animate-pulse-slow';
  else cardStyle = 'bg-gradient-to-br from-fwc-neon/20 to-fwc-gold/5 border-fwc-neon shadow-lg shadow-fwc-neon/20';

  return (
    <div className={`rounded-lg border p-3 transition-all ${cardStyle}`}>
      <div className="flex justify-between items-start mb-2">
        <span className="font-display font-bold text-xs text-white px-2 py-0.5 bg-fwc-bg/50 rounded">{cromo.codigo}</span>
        <span className="text-gray-500 text-xs font-mono">#{String(cromo.numero).padStart(3, '0')}</span>
      </div>
      <div className="flex items-center gap-2 mb-2">
        <Bandera iso={eqInfo.iso} size="md" />
        <p className="font-display font-bold text-sm text-white truncate">{eqInfo.nombre}</p>
      </div>
      {cromo.detalle && <p className="text-xs text-gray-400 truncate mb-2">{cromo.detalle}</p>}
      <div className="text-center pt-2 border-t border-fwc-border/50 mb-2">
        {tienePedidoAprobado ? <span className="text-green-400 text-xs uppercase tracking-wider font-display font-bold">🎁 ¡Apartado!</span> :
         tienePedidoPendiente ? <span className="text-fwc-gold text-xs uppercase tracking-wider font-display font-bold">⏳ Esperando</span> :
         tipo === 'paraMi' ? <span className="text-fwc-gold text-xs uppercase tracking-wider font-display font-bold">⭐ ×{cantidad} disponibles</span> :
         <span className="text-fwc-neon text-xs uppercase tracking-wider font-display font-bold">🎁 Tú ×{cantidad}</span>}
      </div>
      {tipo === 'paraMi' && (
        <>
          {tienePedidoAprobado && onMarcarRecibido && (
            <div className="space-y-1">
              <button onClick={() => onMarcarRecibido(cromo)} disabled={pidiendoEsteCromo === cromo.numero}
                ...
                {pidiendoEsteCromo === cromo.numero ? <Loading className="w-3 h-3 animate-spin" /> : <><ShoppingCart className="w-3 h-3" />Ya lo tengo</>}
              </button>
              <button onClick={() => onCancelar(cromo)} disabled={pidiendoEsteCromo === cromo.numero}
          ...
          {tienePedidoPendiente && (
            <div className="space-y-1">
              <button disabled className="w-full bg-fwc-bg border border-fwc-gold/40 text-fwc-gold font-display font-bold uppercase tracking-wider text-xs py-1.5 rounded cursor-not-allowed">
                ⏳ Solicitado
              </button>
              <button onClick={() => onCancelar(cromo)} disabled={pidiendoEsteCromo === cromo.numero}
                className="w-full border border-fwc-border text-gray-400 hover:border-fwc-accent hover:text-fwc-accent font-display font-bold uppercase tracking-wider text-[10px] py-1 rounded transition-all disabled:opacity-50">
          ...
          {!pedidoActivo && onPedir && (
            <button onClick={() => onPedir(cromo)} disabled={pidiendoEsteCromo === cromo.numero}
              ...
              {pidiendoEsteCromo === cromo.numero ? <Loading className="w-3 h-3 animate-spin" /> : <><ShoppingCart className="w-3 h-3" />Pedir</>}
            </button>
          )}
        </>
      )}
...
function CromoVistaAmigo({ cromo, cantidadAmigo, cantidadMia, onPedir, onMarcarRecibido, onCancelar, pedidoActivo, pidiendoEsteCromo }) {
  ...
  if (tienePedidoAprobado) cardStyle = 'bg-gradient-to-br from-green-500/10 to-fwc-gold/10 border-green-500 shadow-lg shadow-green-500/20';
  else if (tienePedidoPendiente) cardStyle = 'bg-fwc-card border-fwc-gold/40';
  else if (esMatch) cardStyle = 'bg-gradient-to-br from-fwc-gold/20 to-fwc-accent/10 border-fwc-gold shadow-lg shadow-fwc-gold/20 animate-pulse-slow';
  else if (elLoTieneRepe) cardStyle = 'bg-fwc-card border-fwc-neon/40';
  else if (elLoTiene) cardStyle = 'bg-fwc-card border-fwc-gold/30';
  ...
  return (
    <div className={`rounded-lg border p-3 transition-all ${cardStyle}`}>
      ...
      <div className="flex items-center gap-2 mb-2">
        ...
        <p className="font-display font-bold text-sm text-white truncate">{equipoInfo.nombre}</p>
      </div>
      {cromo.detalle && <p className="text-xs text-gray-400 truncate mb-2">{cromo.detalle}</p>}
      <div className="text-center pt-2 border-t border-fwc-border/50 mb-2">
        {!elLoTiene ? <span className="text-gray-500 text-xs uppercase tracking-wider">No lo tiene</span> :
         tienePedidoAprobado ? <span className="text-green-400 text-xs uppercase tracking-wider font-display font-bold">🎁 ¡Apartado para ti!</span> :
         tienePedidoPendiente ? <span className="text-fwc-gold text-xs uppercase tracking-wider font-display font-bold">⏳ Esperando aprobación</span> :
         esMatch ? <span className="text-fwc-gold text-xs uppercase tracking-wider font-display font-bold">⭐ ¡Para ti!</span> :
         elLoTieneRepe ? <span className="text-fwc-neon text-xs uppercase tracking-wider font-bold">×{cantidadAmigo} repes</span> :
         <span className="text-fwc-gold text-xs uppercase tracking-wider font-bold">✓ Tiene 1</span>}
      </div>
      {tienePedidoAprobado && onMarcarRecibido && (
        <div className="space-y-1">
          <button onClick={() => onMarcarRecibido(cromo)} disabled={pidiendoEsteCromo}
            className="w-full bg-green-500 hover:bg-green-600 text-white font-display font-bold uppercase tracking-wider text-xs py-1.5 rounded transition-all flex items-center justify-center gap-1 disabled:opacity-50">
            {pidiendoEsteCromo ? <Loading className="w-3 h-3 animate-spin" /> : <><ShoppingCart className="w-3 h-3" />Ya lo tengo</>}
          </button>
          <button onClick={() => onCancelar(cromo)} disabled={pidiendoEsteCromo}
            className="w-full border border-fwc-border text-gray-400 hover:border-fwc-accent hover:text-fwc-accent font-display font-bold uppercase tracking-wider text-[10px] py-1 rounded transition-all disabled:opacity-50">
            Cancelar
          </button>
        </div>
      ...
      {tienePedidoPendiente && (
        <div className="space-y-1">
          <button disabled className="w-full bg-fwc-bg border border-fwc-gold/40 text-fwc-gold font-display font-bold uppercase tracking-wider text-xs py-1.5 rounded flex items-center justify-center gap-1 cursor-not-allowed">
            ⏳ Solicitado
          </button>
          <button onClick={() => onCancelar(cromo)} disabled={pidiendoEsteCromo}
            className="w-full border border-fwc-border text-gray-400 hover:border-fwc-accent hover:text-fwc-accent font-display font-bold uppercase tracking-wider text-[10px] py-1 rounded transition-all disabled:opacity-50">
      ...
      {!pedidoActivo && esMatch && onPedir && (
        <button onClick={() => onPedir(cromo)} disabled={pidiendoEsteCromo}
          className="w-full bg-fwc-gold hover:bg-yellow-500 text-fwc-bg font-display font-bold uppercase tracking-wider text-xs py-1.5 rounded transition-all flex items-center justify-center gap-1 disabled:opacity-50">
          {pidiendoEsteCromo ? <Loading className="w-3 h-3 animate-spin" /> : <><ShoppingCart className="w-3 h-3" />Pedir</>}
        </button>
      )}
    </div>
  );
